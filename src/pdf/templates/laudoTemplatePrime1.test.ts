...
describe('Laudo Template Prime I — HTML (puro)', () => {
  ...
  it('omite a caixa ICP quando não há assinatura', () => {
    expect(buildLaudoPrime1Html(dadosMockLaudo, QR_FAKE)).not.toContain('ASSINADO DIGITALMENTE');
  });
  ...
  it('omite a seção de anexos quando não há arquivos', () => {
    expect(buildLaudoPrime1Html(dadosMockLaudo, QR_FAKE)).not.toContain('Arquivos Técnicos Anexos');
  });
});
