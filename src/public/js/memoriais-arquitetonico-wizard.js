/* v3.50.0: Wizard UNIFICADO Memorial Arquitetonico (SINAPI/NBR 13532) - 3 passos (Obra->Dados->Geracao). */
(function () {
  'use strict';
  var API = '/api/memoriais/arquitetonico', SLUG = 'arquitetonico';
  ...
  var COR = { verde: '#10b981', azul: '#1F4E79', erro: '#dc2626', alerta: '#d39e00', ac: '#8b5cf6' };
  ...
  var BTN_OK = 'padding:10px 18px;border-radius:8px;border:none;background:' + COR.ac + ';color:#fff;cursor:pointer;font-weight:700;font-size:14px;';
  ...
  var GAUTO = 'display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:0 14px;';
  var PASSOS = [['1', 'Dados da obra'], ['2', 'Programa & acabamentos'], ['3', 'Geracao']];
  ...
  var state, root;
  function defaults() { return { passo: 1, obra: { titulo: 'Residencia Unifamiliar', endereco: '', municipio: 'Acailandia', uf: 'MA', proprietario: '', cpfCnpj: '', areaM2: 0, nPavimentos: 1, prancha: 'PA-01', trtNumero: '' }, uso: { tipoUso: 'residencial', padraoAcabamento: 'normal', nQuartos: 3, nBanheiros: 2, areaMolhadaM2: 18, peDireitoM: 2.7, tipoCobertura: 'laje_telha', acessibilidade: false }, resumo: null, gerado: null }; }
  ...
  function stepper() { return h('div', { style: 'display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap;' }, PASSOS.map(function (p) { var n = parseInt(p[0], 10), at = state.passo === n, fz = state.passo > n; return h('div', { style: 'flex:1;min-width:96px;padding:7px 6px;border-radius:8px;text-align:center;font-size:11px;font-weight:600;background:' + (at ? COR.ac : fz ? 'rgba(139,92,246,.18)' : 'var(--bg-elev,#1c1c1c)') + ';color:' + (at ? '#fff' : fz ? COR.ac : 'var(--text-muted,#888)') + ';border:1px solid var(--border,#333);' }, [(fz ? '✓ ' : n + '. ') + p[1]]); })); }
  function obraCard() { var o = state.obra, card = h('div', { style: CARD }); card.appendChild(h('p', { style: 'margin:0 0 12px;font-weight:600;' }, ['Passo 1 - Dados da obra (vao no cabecalho do PDF)'])); card.appendChild(campo('Titulo da obra *', o.titulo, function (v) { o.titulo = v; })); card.appendChild(campo('Endereco', o.endereco, function (v) { o.endereco = v; }, { placeholder: 'Loteamento ..., Quadra X, Lote Y' })); var g1 = h('div', { style: 'display:grid;grid-template-columns:2fr 1fr;gap:0 14px;' }); g1.appendChild(campo('Municipio', o.municipio, function (v) { o.municipio = v; })); g1.appendChild(campo('UF', o.uf, function (v) { o.uf = v; }, { maxlength: '2' })); card.appendChild(g1); var g2 = h('div', { style: 'display:grid;grid-template-columns:1fr 1fr;gap:0 14px;' }); g2.appendChild(campo('Proprietario *', o.proprietario, function (v) { o.proprietario = v; })); g2.appendChild(campo('CPF/CNPJ', o.cpfCnpj, function (v) { o.cpfCnpj = v; })); card.appendChild(g2); var g3 = h('div', { style: GAUTO }); g3.appendChild(campo('Area construida (m2) *', o.areaM2, function (v) { o.areaM2 = num(v); }, { type: 'number', step: '0.01' })); g3.appendChild(campo('No pavimentos', o.nPavimentos, function (v) { o.nPavimentos = num(v, 1); }, { type: 'number' })); g3.appendChild(campo('Prancha', o.prancha, function (v) { o.prancha = v; })); g3.appendChild(campo('TRT/ART no', o.trtNumero, function (v) { o.trtNumero = v; })); card.appendChild(g3); card.appendChild(h('p', { style: 'margin:4px 0 0;font-size:11px;color:var(--text-muted,#888);' }, ['* Obrigatorios para gerar o memorial.'])); return card; }
  function podeAvancar() { if (state.passo === 1) { var o = state.obra; if (!o.titulo || !o.titulo.trim()) { alert('Informe o titulo da obra.'); return false; } if (!o.proprietario || !o.proprietario.trim()) { alert('Informe o proprietario.'); return false; } if (!(num(o.areaM2) > 0)) { alert('Informe a area construida.'); return false; } } return true; }
  function navBtns() { var box = h('div', { style: 'display:flex;justify-content:space-between;margin-top:16px;' }); box.appendChild(h('button', { style: BTN, onclick: function () { if (state.passo > 1) { state.passo--; render(); } else { fechar(); } } }, [state.passo > 1 ? '< Voltar' : 'Fechar'])); box.appendChild(state.passo < 3 ? h('button', { style: BTN_OK, onclick: function () { if (!podeAvancar()) return; state.passo++; render(); if (state.passo === 3) recalc(); } }, ['Avancar >']) : h('span', {}, [''])); return box; }
  function render() { root.innerHTML = ''; var wrap = h('div', { style: 'max-width:920px;margin:0 auto;' }); var head = h('div', { style: 'display:flex;justify-content:space-between;align-items:center;' }); head.appendChild(h('h2', { style: 'margin:0 0 4px;' }, ['Memorial Arquitetonico - SINAPI / NBR 13532'])); head.appendChild(h('button', { style: BTN, onclick: fechar }, ['← Voltar a lista'])); wrap.appendChild(head); wrap.appendChild(h('p', { style: 'margin:0 0 14px;font-size:12px;color:var(--text-muted,#888);' }, ['Wizard unico: obra, programa/acabamentos e geracao dos 2 PDFs.'])); wrap.appendChild(stepper()); wrap.appendChild(state.passo === 1 ? obraCard() : state.passo === 2 ? renderDados() : renderGeracao()); wrap.appendChild(navBtns()); root.appendChild(wrap); }
  function renderDados() {
    var u = state.uso, card = h('div', { style: CARD });
    card.appendChild(h('p', { style: 'margin:0 0 12px;font-weight:600;' }, ['Passo 2 - Programa e acabamentos']));
  ...
  function renderGeracao() {
    ...
    var s = r.saida, card = h('div', { style: CARD });
    var linhas = [['Area de piso', fmt(r.totais.areaPisoM2, 0) + ' m2'], ['Parede interna', fmt(s.areas.parede_interna_m2, 0) + ' m2'], ['Azulejo', fmt(s.areas.area_molhada_revest_m2, 0) + ' m2'], ['Forro', fmt(s.areas.forro_m2, 0) + ' m2'], ['Pintura', fmt(r.totais.areaPinturaM2, 0) + ' m2'], ['Cobertura', fmt(s.areas.cobertura_m2, 0) + ' m2'], ['Portas', (s.esquadrias.portas_internas + s.esquadrias.portas_externas)], ['Janelas', s.esquadrias.janelas]];
    card.appendChild(h('div', { style: 'display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;' }, linhas.map(function (l) { return h('div', { html: '<div style="font-size:11px;color:var(--text-muted,#888);">' + l[0] + '</div><div style="font-size:16px;color:' + COR.ac + ';font-weight:600;">' + l[1] + '</div>' }); })));
    var sn = r.statusNormativo || {}, checks = [['Pe-direito >= 2,50 m', sn.peDireitoMinimoOK], ['Iluminacao/ventilacao (1/8)', sn.ventilacaoIluminacaoOK], ['Acessibilidade prevista', sn.acessibilidadePrevista], ['Cobertura definida', sn.coberturaDefinida]];
  ...
  function blocoGerado() { var g = state.gerado, card = h('div', { style: CARD + 'border-color:' + COR.ac + ';' }); card.appendChild(h('p', { style: 'margin:0 0 10px;font-weight:600;color:' + COR.ac + ';' }, ['✅ Memorial #' + (g.codigo || g.id) + ' gerado e salvo'])); var links = h('div', { style: 'display:flex;gap:10px;flex-wrap:wrap;margin-bottom:12px;' }); links.appendChild(h('a', { href: g.urlMemorial, target: '_blank', style: BTN + 'text-decoration:none;' }, ['Memorial PDF'])); links.appendChild(h('a', { href: g.urlQuantitativo, target: '_blank', style: BTN + 'text-decoration:none;' }, ['Lista PDF'])); card.appendChild(links); var tel = h('input', { style: INP + 'max-width:200px;display:inline-block;width:auto;', placeholder: 'WhatsApp (DDD+numero)' }); card.appendChild(h('div', { style: 'display:flex;gap:8px;align-items:center;flex-wrap:wrap;' }, [tel, h('button', { style: BTN, onclick: function () { api('/' + g.id + '/enviar-whatsapp', { telefone: tel.value.trim() }).then(function () { alert('Enviado!'); }).catch(function (e) { alert(e.message); }); } }, ['Enviar WhatsApp'])])); card.appendChild(h('button', { style: BTN + 'margin-top:14px;', onclick: fechar }, ['Concluir e voltar a lista'])); return card; }
  function abrir(prefill, passo) { var view = document.getElementById('view-memoriais'); if (!view) return; view.setAttribute('data-mem' + SLUG + '-active', '1'); montar(view, { prefill: prefill || {}, passo: passo || 1 }); }
  (window.MemWizards = window.MemWizards || {})[SLUG] = function () { abrir({}, 1); };
  ...
  window.MemArqWizard = { montar: montar, abrir: abrir };
})();

/* ===== v3.51.1 — aba VTA INLINE (abre na area de conteudo, igual VTO) ===== */
(function () {
  'use strict';
  var TOOLS = [
  ...
  function montarView() {
    var ref = document.querySelector('.view');
    if (!ref || !ref.parentNode) return null;
    var vv = document.getElementById('view-vta');
    if (vv) return vv;
    vv = vh('div', { id: 'view-vta', 'class': 'view' });
    var card = vh('div', { 'class': 'card' });
    card.appendChild(vh('h2', { style: 'margin:0 0 4px;' }, ['🏗️ VTA — Vistoria Técnica de As-Built']));
    card.appendChild(vh('p', { style: 'margin:0 0 16px;font-size:12px;color:var(--text-muted,#8a93a6);' }, ['Levantamento de edificação existente para regularização (averbação, habite-se, REURB, georreferenciamento).']));
    var grid = vh('div', { style: 'display:grid;grid-template-columns:1fr;gap:14px;' });
    if (window.innerWidth > 640) grid.style.gridTemplateColumns = '1fr 1fr';
    TOOLS.forEach(function (t) {
      var a = vh('a', { href: t.href, style: 'display:block;text-decoration:none;border:1px solid var(--border,#2a2f3a);border-left:4px solid ' + t.cor + ';border-radius:12px;padding:18px;background:var(--bg-elev,#171a21);color:var(--text,#e8eaed);' });
      a.appendChild(vh('div', { style: 'font-size:32px;margin-bottom:8px;' }, [t.icon]));
      a.appendChild(vh('div', { style: 'font-size:16px;font-weight:700;margin-bottom:4px;' }, [t.titulo]));
      a.appendChild(vh('div', { style: 'font-size:12px;color:var(--text-muted,#8a93a6);line-height:1.5;' }, [t.desc]));
      a.appendChild(vh('div', { style: 'margin-top:10px;font-size:13px;color:' + t.link + ';' }, ['Abrir →']));
      grid.appendChild(a);
    });
    card.appendChild(grid);
    card.appendChild(vh('p', { style: 'margin:14px 0 0;font-size:11px;color:var(--text-muted,#5b6478);' }, ['Dica: abre melhor no celular/tablet (câmera + GPS).']));
    vv.appendChild(card);
    ref.parentNode.appendChild(vv);
    return vv;
  }
  function ligar() {
    ...
    if (!btn) {
      btn = vh('button', { 'class': 'tab', 'data-tab': 'vta', title: 'Vistoria Técnica de As-Built' }, ['🏗️ VTA']);
    ...
    btn.addEventListener('click', function (e) {
      ...
      var vv = montarView(); if (!vv) return;
