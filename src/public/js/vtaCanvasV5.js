/**
 * vtaCanvasV5.js — Motor CAD As-Built v5 (estilo Revit) para VTA
 * Romatec Consultoria Total — ZAYRA
 * V4 + correções: orto delta (trava H/V), parede 2-cliques imediata,
 * grupo MODIFICAR (mover/rotacionar/espelhar/cortar/unir/chanfro).
...
const CanvasEngine = (() => {
  ...
  const MODIFY = ['move', 'rotate', 'mirror', 'trim', 'extend', 'fillet'];
  ...
  let orthoMode = false;
  let distBuffer = '', distActive = false;
  ...
  // helpers de modificação (geometria pura — validada)
  function rotatePt(p, piv, deg) { const r = deg * Math.PI / 180, c = Math.cos(r), s = Math.sin(r), dx = p.x - piv.x, dy = p.y - piv.y; return { x: piv.x + dx * c - dy * s, y: piv.y + dx * s + dy * c }; }
  function mirrorPt(p, a, b) { const dx = b.x - a.x, dy = b.y - a.y, L2 = dx * dx + dy * dy || 1e-9; const t = ((p.x - a.x) * dx + (p.y - a.y) * dy) / L2; const fx = a.x + t * dx, fy = a.y + t * dy; return { x: 2 * fx - p.x, y: 2 * fy - p.y }; }
  function lineLineIntersect(w1, w2) { return lineIntersect({ x: w1.x1, y: w1.y1 }, { x: w1.x2, y: w1.y2 }, { x: w2.x1, y: w2.y1 }, { x: w2.x2, y: w2.y2 }); }
  ...
  function toggleOrtho() {
    ...
    showToast('Orto ' + (orthoMode ? 'ATIVO' : 'desligado'), 800); redraw();
  }
  ...
  function drawGhost() {
    ...
    // preview de modificação
    if (modify && modify.phase === 2) {
      ctx.strokeStyle = COL.dim; ctx.setLineDash([4, 3]); ctx.lineWidth = 1;
      let from = modify.tool === 'rotate' ? modify.piv : (modify.tool === 'mirror' ? modify.a : modify.base);
      if (from) { const a = worldToScreen(from.x, from.y), b = worldToScreen(pointer.wx, pointer.wy); ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y); ctx.stroke(); }
      ctx.setLineDash([]);
    }
  }
  ...
  function updateHud() {
    const z = document.getElementById('zoom'); if (z) z.textContent = Math.round(state.zoom * 100) + '%' + (orthoMode ? ' · ORTO' : '') + (modify ? ' · ' + modify.tool.toUpperCase() : '');
  }
  let toastTimer = null;
  ...
  function undo() { if (!undoStack.length) { showToast('Nada para desfazer', 900); return; } redoStack.push(snapshot()); applySnap(undoStack.pop()); selected = null; modify = null; closeWallPanel(); redraw(); }
  function redo() { if (!redoStack.length) return; undoStack.push(snapshot()); applySnap(redoStack.pop()); selected = null; redraw(); }
  function clearAll() { if (!confirm('Limpar todo o croqui?')) return; pushHistory(); state.walls = []; state.openings = []; state.rooms = []; state.dimensions = []; state.texts = []; wallChain = dimDraft = measureDraft = null; selected = null; modify = null; closeWallPanel(); redraw(); }
  ...
  // ─── MODIFICAR ───────────────────────────────────────────────────────────────
  function transformEl(hit, fn) {
    if (hit.kind === 'wall') { const w = state.walls.find(x => x.id === hit.id); if (!w) return; const a = fn({ x: w.x1, y: w.y1 }), b = fn({ x: w.x2, y: w.y2 }); w.x1 = a.x; w.y1 = a.y; w.x2 = b.x; w.y2 = b.y; }
    else if (hit.kind === 'text') { const t = state.texts.find(x => x.id === hit.id); if (!t) return; const p = fn({ x: t.x, y: t.y }); t.x = p.x; t.y = p.y; }
    else if (hit.kind === 'dimension') { const d = state.dimensions.find(x => x.id === hit.id); if (!d) return; const a = fn({ x: d.x1, y: d.y1 }), b = fn({ x: d.x2, y: d.y2 }); d.x1 = a.x; d.y1 = a.y; d.x2 = b.x; d.y2 = b.y; }
  }
  function pickModifyWall(wx, wy) { const h = pickElement(wx, wy); return h && h.kind === 'wall' ? state.walls.find(w => w.id === h.id) : null; }
  function modifyDown(tool, wx, wy, snap) {
    const tx = snap ? snap.x : wx, ty = snap ? snap.y : wy;
    if (tool === 'move' || tool === 'rotate' || tool === 'mirror') {
      if (!modify) { const hit = pickElement(wx, wy); if (!hit || (hit.kind !== 'wall' && hit.kind !== 'text' && hit.kind !== 'dimension')) { showToast('Selecione parede/texto/cota', 1300); return; } modify = { tool, el: hit, phase: 1 }; selected = hit; showToast(tool === 'move' ? 'Clique o ponto base' : (tool === 'rotate' ? 'Clique o pivô' : 'Clique o 1º ponto do eixo'), 1400); redraw(); return; }
      if (tool === 'move') {
        if (modify.phase === 1) { modify.base = { x: tx, y: ty }; modify.phase = 2; showToast('Clique o destino (ou digite a distância)', 1400); }
        else { pushHistory(); const dx = tx - modify.base.x, dy = ty - modify.base.y; transformEl(modify.el, p => ({ x: p.x + dx, y: p.y + dy })); modify = null; redraw(); }
      } else if (tool === 'rotate') {
        if (modify.phase === 1) { modify.piv = { x: tx, y: ty }; modify.phase = 2; showToast('Clique o ângulo (ou digite graus + Enter)', 1600); }
        else { pushHistory(); const ang = Math.atan2(ty - modify.piv.y, tx - modify.piv.x) * 180 / Math.PI; transformEl(modify.el, p => rotatePt(p, modify.piv, ang)); modify = null; redraw(); }
      } else {
        if (modify.phase === 1) { modify.a = { x: tx, y: ty }; modify.phase = 2; showToast('Clique o 2º ponto do eixo', 1400); }
        else {
          const b = { x: tx, y: ty }; const keep = confirm('Espelhar: manter o original?\n[OK] duplica · [Cancelar] move'); pushHistory();
          if (keep && modify.el.kind === 'wall') { const w = state.walls.find(x => x.id === modify.el.id); const a2 = mirrorPt({ x: w.x1, y: w.y1 }, modify.a, b), b2 = mirrorPt({ x: w.x2, y: w.y2 }, modify.a, b); state.walls.push({ id: uid('w'), x1: a2.x, y1: a2.y, x2: b2.x, y2: b2.y, thickness: w.thickness }); }
          else transformEl(modify.el, p => mirrorPt(p, modify.a, b));
          modify = null; redraw();
        }
      }
      return;
    }
    // trim / extend / fillet (2 paredes)
    if (!modify) { const w = pickModifyWall(wx, wy); if (!w) { showToast('Clique numa parede', 1200); return; } modify = { tool, w1: w, click1: { x: wx, y: wy } }; selected = { kind: 'wall', id: w.id }; showToast(tool === 'trim' ? 'Clique a parede a cortar (lado a remover)' : (tool === 'extend' ? 'Clique a parede a estender' : 'Clique a 2ª parede'), 1600); redraw(); return; }
    const w2 = pickModifyWall(wx, wy); if (!w2 || w2.id === modify.w1.id) { showToast('Clique na 2ª parede', 1200); return; }
    const ix = lineLineIntersect(modify.w1, w2); if (!ix) { showToast('Paredes paralelas — sem interseção', 1600); modify = null; redraw(); return; }
    pushHistory();
    if (tool === 'trim') { const d1 = dist(wx, wy, w2.x1, w2.y1), d2 = dist(wx, wy, w2.x2, w2.y2); if (d1 < d2) { w2.x1 = ix.x; w2.y1 = ix.y; } else { w2.x2 = ix.x; w2.y2 = ix.y; } }
    else if (tool === 'extend') { const d1 = dist(w2.x1, w2.y1, ix.x, ix.y), d2 = dist(w2.x2, w2.y2, ix.x, ix.y); if (d1 < d2) { w2.x1 = ix.x; w2.y1 = ix.y; } else { w2.x2 = ix.x; w2.y2 = ix.y; } }
    else { const w1 = modify.w1; const e1a = dist(w1.x1, w1.y1, ix.x, ix.y), e1b = dist(w1.x2, w1.y2, ix.x, ix.y); if (e1a < e1b) { w1.x1 = ix.x; w1.y1 = ix.y; } else { w1.x2 = ix.x; w1.y2 = ix.y; } const e2a = dist(w2.x1, w2.y1, ix.x, ix.y), e2b = dist(w2.x2, w2.y2, ix.x, ix.y); if (e2a < e2b) { w2.x1 = ix.x; w2.y1 = ix.y; } else { w2.x2 = ix.x; w2.y2 = ix.y; } }
    modify = null; redraw();
  }

  function onToolDown(wx, wy, snap) {
    if (MODIFY.includes(activeTool)) { modifyDown(activeTool, wx, wy, snap); return; }
  ...
  function finishWall() { wallChain = null; redraw(); }
  function bbox() {
  ...
  function onMove(e) {
    ...
    else if (MODIFY.includes(activeTool)) snapPt = findSnap(rp);
    else snapPt = null;
    redraw();
  }
  ...
  function onDbl(e) {
    ...
    if (activeTool === 'wall' && wallChain) { finishWall(); return; }
  ...
  function showDistInput() {
    ...
    const unit = (modify && modify.tool === 'rotate') ? '°' : ' cm';
    document.getElementById('distInputValue').textContent = (distBuffer || '0') + unit;
  ...
  function onKey(e) {
    ...
    // ângulo digitado para ROTACIONAR
    if (!typing && modify && modify.tool === 'rotate' && modify.phase === 2) {
      if ((e.key >= '0' && e.key <= '9') || e.key === '.' || e.key === ',' || e.key === '-') { distBuffer += (e.key === ',' ? '.' : e.key); distActive = true; showDistInput(); e.preventDefault(); return; }
      if (e.key === 'Backspace' && distActive) { distBuffer = distBuffer.slice(0, -1); showDistInput(); e.preventDefault(); return; }
      if (e.key === 'Enter' && distActive && distBuffer) { const ang = parseFloat(distBuffer); if (!isNaN(ang)) { pushHistory(); transformEl(modify.el, p => rotatePt(p, modify.piv, ang)); } modify = null; cancelDist(); redraw(); e.preventDefault(); return; }
    }
    // input de distância (parede)
    ...
    if (e.key === 'Escape') { if (distActive) { cancelDist(); return; } if (modify) { modify = null; showToast('Modificação cancelada', 800); redraw(); return; } wallChain = dimDraft = measureDraft = null; hideOpeningPanel(); closeWallPanel(); selected = null; redraw(); return; }
  ...
  function setTool(tool) {
    activeTool = tool; wallChain = dimDraft = measureDraft = null; hideOpeningPanel(); closeWallPanel(); cancelDist(); selected = null; modify = null;
    document.querySelectorAll('.tool-btn[data-tool]').forEach(b => b.classList.toggle('active', b.dataset.tool === tool));
    if (wrap) wrap.className = 'tool-' + (MODIFY.includes(tool) ? 'modify' : tool);
    redraw();
  }
  ...
  return {
    ...
    rotatePt, mirrorPt, lineLineIntersect,
...
// ── fim do vtaCanvasV5.js (Romatec / ZAYRA) — motor CAD As-Built v5 ──
