...
(function () {
  ...
  var COR = { verde: '#10b981', azul: '#1F4E79', erro: '#dc2626', alerta: '#d39e00', ac: '#f5b301' };
  ...
  var PASSOS = [['1', 'Dados da obra'], ['2', 'Dados eletricos'], ['3', 'Geracao']];
  var state, root;
  ...
  function stepper() { return h('div', { style: 'display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap;' }, PASSOS.map(function (p) { var n = parseInt(p[0], 10), at = state.passo === n, fz = state.passo > n; return h('div', { style: 'flex:1;min-width:96px;padding:7px 6px;border-radius:8px;text-align:center;font-size:11px;font-weight:600;background:' + (at ? COR.ac : fz ? 'rgba(245,179,1,.18)' : 'var(--bg-elev,#1c1c1c)') + ';color:' + (at ? '#3a2c00' : fz ? COR.ac : 'var(--text-muted,#888)') + ';border:1px solid var(--border,#333);' }, [(fz ? '✓ ' : n + '. ') + p[1]]); })); }
  function obraCard() {
  ...
  function render() {
    ...
    wrap.appendChild(state.passo === 1 ? obraCard() : state.passo === 2 ? renderDados() : renderGeracao());
