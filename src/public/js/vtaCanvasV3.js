/**
 * vtaCanvasV3.js — Motor CAD As-Built v3 (estilo Revit) para VTA
 * Romatec Consultoria Total — ZAYRA
 * HTML5 Canvas 2D puro, zero dependências. Coordenadas internas em CENTÍMETROS.
 * Tela = mundo_cm * PX_PER_CM + pan. Persistência via /api/canvas. Voltar: /obras?vta=1.
 */
const CanvasEngine = (() => {
  'use strict';
  ...
  function projSeg(px, py, ax, ay, bx, by) {
    ...
    return { t, x: ax + t * vx, y: ay + t * vy, d: dist(px, py, ax + t * vx, ay + t * vy) };
  }
  ...
  function snapCandidates() {
    const out = [];
    state.walls.forEach(w => {
      out.push({ x: w.x1, y: w.y1, type: 'wallEndpoint' });
      out.push({ x: w.x2, y: w.y2, type: 'wallEndpoint' });
      out.push({ x: (w.x1 + w.x2) / 2, y: (w.y1 + w.y2) / 2, type: 'wallMidpoint' });
    });
    if (wallChain && wallChain.pts.length) { const p0 = wallChain.pts[0]; out.push({ x: p0.x, y: p0.y, type: 'chainStart' }); }
    return out;
  }
  function findSnap(mousePx) {
    let best = null, bestD = SNAP_RADIUS_PX;
    snapCandidates().forEach(c => { const s = worldToScreen(c.x, c.y); const d = dist(mousePx.x, mousePx.y, s.x, s.y); if (d < bestD) { bestD = d; best = c; } });
    return best;
  }
  function drawSnapIndicator() {
    if (!snapPt) return;
    const s = worldToScreen(snapPt.x, snapPt.y);
    ctx.strokeStyle = COL.snap; ctx.lineWidth = 1.5; ctx.setLineDash([]);
    ...
    else { ctx.beginPath(); ctx.moveTo(s.x - 4, s.y - 4); ctx.lineTo(s.x + 4, s.y + 4); ctx.moveTo(s.x + 4, s.y - 4); ctx.lineTo(s.x - 4, s.y + 4); ctx.stroke(); }
  }
  ...
  function makeRoomFromChain(pts) {
    ...
    showToast('Ambiente criado: ' + area.toFixed(2).replace('.', ',') + ' m² — 2 cliques p/ nomear', 2200);
  }
  function drawRooms() {
  ...
  function drawGhost() {
    if (wallChain && wallChain.pts.length) {
      ...
      const cur = snapPt || { x: pointer.wx, y: pointer.wy }; const sc = worldToScreen(cur.x, cur.y); ctx.lineTo(sc.x, sc.y);
      ...
      ctx.fillText(fmtM(dist(last.x, last.y, cur.x, cur.y)) + ' m', sc.x + 8, sc.y - 6);
    }
  ...
  function drawSelection() {
    ...
    const q = getWallPolygon(w).map(p => worldToScreen(p.x, p.y));
    ctx.strokeStyle = COL.select; ctx.lineWidth = 2; ctx.setLineDash([5, 3]);
    ctx.beginPath(); ctx.moveTo(q[0].x, q[0].y); q.forEach(p => ctx.lineTo(p.x, p.y)); ctx.closePath(); ctx.stroke(); ctx.setLineDash([]);
  }
  function updateHud() { const z = document.getElementById('zoom'); if (z) z.textContent = Math.round(state.zoom * 100) + '%'; }
  let toastTimer = null;
  ...
  function undo() { if (!undoStack.length) { showToast('Nada para desfazer', 900); return; } redoStack.push(snapshot()); applySnap(undoStack.pop()); selected = null; redraw(); }
  function redo() { if (!redoStack.length) return; undoStack.push(snapshot()); applySnap(redoStack.pop()); selected = null; redraw(); }
  function clearAll() { if (!confirm('Limpar todo o croqui?')) return; pushHistory(); state.walls = []; state.openings = []; state.rooms = []; state.dimensions = []; state.texts = []; wallChain = dimDraft = measureDraft = null; selected = null; redraw(); }
  ...
  function pickElement(wx, wy) {
    ...
    for (const w of state.walls) { const pr = projSeg(wx, wy, w.x1, w.y1, w.x2, w.y2); if (cmToPx(pr.d) < Math.max(8, cmToPx(w.thickness / 2))) return { kind: 'wall', id: w.id }; }
    return null;
  }
  ...
  function onToolDown(wx, wy, snap) {
    ...
    switch (activeTool) {
      case 'wall': {
        if (!wallChain) { wallChain = { pts: [{ x: tx, y: ty }], thickness: state.wallThickness }; break; }
        const p0 = wallChain.pts[0]; const sp = worldToScreen(p0.x, p0.y), sc = worldToScreen(tx, ty);
        const closing = wallChain.pts.length >= 3 && dist(sp.x, sp.y, sc.x, sc.y) <= SNAP_RADIUS_PX;
        if (closing) { finishWall(true); } else wallChain.pts.push({ x: tx, y: ty });
        break;
      }
      case 'door': case 'window': {
        ...
        const s = worldToScreen(wu.x, wu.y);
        showOpeningPanel(activeTool, s.x + 12, s.y + 12, null);
        break;
      }
      case 'dim': {
        ...
        const toC = { x: bb.cx - mid.x, y: bb.cy - mid.y };
        if (dir.x * toC.x + dir.y * toC.y > 0) dir = { x: -dir.x, y: -dir.y };
        pushHistory();
      ...
      case 'erase': { const hit = pickElement(wx, wy); if (hit) { pushHistory(); removeElement(hit); selected = null; redraw(); } break; }
      case 'select': {
        const hit = pickElement(wx, wy); selected = hit;
  ...
  function onUp() { panDrag = null; if (wrap) wrap.classList.remove('panning'); }
  function onDbl(e) {
  ...
  function onKey(e) {
    ...
    if (e.key === 'Escape') { wallChain = dimDraft = measureDraft = null; hideOpeningPanel(); selected = null; redraw(); }
    if ((e.key === 'Delete' || e.key === 'Backspace') && selected) { pushHistory(); removeElement(selected); selected = null; redraw(); }
    const map = { s: 'select', h: 'pan', w: 'wall', p: 'door', j: 'window', c: 'dim', t: 'text', m: 'measure', n: 'north', e: 'erase' };
    if (map[e.key.toLowerCase()]) setTool(map[e.key.toLowerCase()]);
  }
  function setTool(tool) {
    activeTool = tool; wallChain = dimDraft = measureDraft = null; hideOpeningPanel(); selected = null;
  ...
  function save() {
    ...
    })).then(() => { showToast('Salvo com sucesso ✓', 1400); return canvasId; }).catch(e => { showToast('Erro ao salvar: ' + e.message, 2200); throw e; }).finally(() => setLoading(false));
  }
  ...
  function bind() {
    ...
    on('btn-voltar', () => { location.href = '/obras?vta=1'; });
  ...
  function init() {
    ...
    state.pan = { x: 80, y: 80 };
    loadExisting().finally(() => { redraw(); showToast('Canvas pronto ✓', 1000); });
  }

  return {
    init, undo, redo, clearAll, exportPNG, save, gerarPrancha, enviarWhatsapp, setTool, openTables, resizeCanvas, redraw,
    cmToPx, pxToCm, worldToScreen, screenToWorld, getWallPolygon, lineIntersect, shoelaceArea,
    findSnap, serializeState, restoreState, buildDoorsTable, buildWindowsTable, buildRoomsTable, addWall, clearWalls,
    get _state() { return state; }, get _canvas() { return canvas; },
  };
})();
...
// ── fim do vtaCanvasV3.js (Romatec / ZAYRA) — motor CAD As-Built v3 ──
